...
import com.segment.lib.helper.LayoutHelper;
...
public class ActionBarLayout extends FrameLayout {
    public interface ActionBarLayoutDelegate {
        ...
        boolean needPresentFragment(BaseSegment fragment, boolean removeLast, boolean forceWithoutAnimation, ActionBarLayout layout);

        boolean needAddFragmentToStack(BaseSegment fragment, ActionBarLayout layout);
    ...
    public void onConfigurationChanged(android.content.res.Configuration newConfig) {
        ...
        if (!fragmentsStack.isEmpty()) {
            BaseSegment lastFragment = fragmentsStack.get(fragmentsStack.size() - 1);
...
//            currentActionBar = lastFragment.actionBar;
            ...
            BaseSegment lastFragment = fragmentsStack.get(fragmentsStack.size() - 2);
            lastFragment.onPause();
...
//            fragmentView.setBackgroundColor(Theme.getColor(Theme.key_windowBackgroundWhite));
        ...
        if (!checkTransitionAnimation() && !inActionMode && !animationInProgress) {
            if (fragmentsStack.size() > 1) {
                if (ev != null && ev.getAction() == MotionEvent.ACTION_DOWN && !startedTracking && !maybeStartTracking) {
                    BaseSegment currentFragment = fragmentsStack.get(fragmentsStack.size() - 1);
    ...
    private void presentFragmentInternalRemoveOld(boolean removeLast, final BaseSegment fragment) {
        if (fragment == null) {
            return;
        }
        fragment.onPause();
    ...
    public boolean presentFragment(final BaseSegment fragment, final boolean removeLast, boolean forceWithoutAnimation, boolean check) {
        ...
        final BaseSegment currentFragment = !fragmentsStack.isEmpty() ? fragmentsStack.get(fragmentsStack.size() - 1) : null;
        ...
        if (fragment.hasBottomBar() && getBottomBar() != null) {
            LinearLayout.LayoutParams llParams = new LinearLayout.LayoutParams(LayoutHelper.MATCH_PARENT, getScreenHeight() - getBottomBarHeight());

            containerViewBack.addView(fragmentView, llParams);

            View bottomBar = getBottomBar();
            if (bottomBar != null && bottomBar.getParent() == null) {
                LinearLayout.LayoutParams bottomBarParam = new LinearLayout.LayoutParams(LayoutHelper.MATCH_PARENT, getBottomBarHeight());
                containerViewBack.addView(bottomBar, bottomBarParam);
            }
//            else {
...
//        }
    ...
    public boolean addFragmentToStack(BaseSegment fragment) {
        return addFragmentToStack(fragment, -1);
    }

    public boolean addFragmentToStack(BaseSegment fragment, int position) {
        ...
        if (position == -1) {
            if (!fragmentsStack.isEmpty()) {
                BaseSegment previousFragment = fragmentsStack.get(fragmentsStack.size() - 1);
    ...
    private void closeLastFragmentInternalRemoveOld(BaseSegment fragment) {
        fragment.onPause();
        fragment.onFragmentDestroy();
        fragment.setParentLayout(null);
        fragmentsStack.remove(fragment);
    ...
    public void closeLastFragment(boolean animated) {
        ...
        final BaseSegment currentFragment = fragmentsStack.get(fragmentsStack.size() - 1);
        BaseSegment previousFragment = null;
        if (fragmentsStack.size() > 1) {
...
//            }
            ...
            if (needAnimation) {
                transitionAnimationStartTime = System.currentTimeMillis();
                transitionAnimationInProgress = true;
                final BaseSegment previousFragmentFinal = previousFragment;
    ...
    public void showLastFragment() {
        ...
        for (int a = 0; a < fragmentsStack.size() - 1; a++) {
            BaseSegment previousFragment = fragmentsStack.get(a);
            if (previousFragment.fragmentView != null) {
                ViewGroup parent = (ViewGroup) previousFragment.fragmentView.getParent();
                if (parent != null) {
                    previousFragment.onPause();
...
//        }
    ...
    private void removeFragmentFromStackInternal(BaseSegment fragment) {
        fragment.onPause();
        fragment.onFragmentDestroy();
        fragment.setParentLayout(null);
        fragmentsStack.remove(fragment);
    ...
    public void removeFragmentFromStack(BaseSegment fragment) {
        if (useAlphaAnimations && fragmentsStack.size() == 1) {
            closeLastFragment(true);
        } else {
